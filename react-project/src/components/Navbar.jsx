...
function Navbar({ onLoginClick, isAuthenticated, onLogout, user }) {
  ...
  const toggleCart = () => {
    ...
    console.log('Current isCartOpen:', isCartOpen);
    console.log('Cart items count:', getCartCount());
    setIsCartOpen(prev => {
      console.log('Setting isCartOpen to:', !prev);
      return !prev;
    });
  };

  useEffect(() => {
    console.log('🔄 isCartOpen changed to:', isCartOpen);
  }, [isCartOpen]);
